...
public class Command
{
        ...
        dispatcher.register(
                Commands.literal("dumpRegistry")
                        .executes(ctx -> {
                            ...
                            return runScopedDump(server, ctx.getSource(), RegistryDumper.DumpScope.FULL);
                        })
    ...
    public static int runScopedDump(
            MinecraftServer server,
            CommandSourceStack s,
            RegistryDumper.DumpScope scope
    ) {
        ...
        try {
            session = beginDump(server, scope);
        } catch (IOException e) {
            TCompanion.LOGGER.error("Failed to create dump session", e);
        ...
        try {
            if (scope.isFull()) {
                count = runFullPipeline(server, session, s);
            } else if (scope.getType().equals("section")) {
                count = runSectionPipeline(server, session, s);
            } else {
                count = runFullPipeline(server, session, s);
            }
            runPatches(session);
            ...
            abandonDump(session);
            TCompanion.LOGGER.error("Registry dump failed", t);
    ...
    private static int runFullPipeline(
            MinecraftServer server,
            RegistryDumper.DumpSession session,
            CommandSourceStack s
    ) throws Exception {
        Future<RegistryDumper.AtlasDumpResult> iconFuture =
                CompanionSocketBridge.ICON_EXECUTOR.submit(() -> RegistryDumper.dumpIcons(session));
        ...
        RegistryDumper.AtlasDumpResult iconResult;
        try {
            iconResult = iconFuture.get();
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new IOException("Interrupted waiting for icon dump", e);
        } catch (ExecutionException e) {
            throw new IOException("Icon dump failed", e.getCause());
        }
        RegistryDumper.applyIconManifest(session, iconResult);
        RegistryDumper.writeAtlasIndex(session, iconResult.entries());
        send(s, iconResult.count(), "Icons");
        send(s, textures, "Textures");

        return kubejs + iconResult.count() + biome +
                configFeature +
                placeFeature +
                struct +
                structSet +
    ...
    private static int runSectionPipeline(
            MinecraftServer server,
            RegistryDumper.DumpSession session,
            CommandSourceStack s
    ) throws Exception {
    ...
    private static int runSingleSectionDump(
        ...
        try {
            ...
            abandonDump(session);
            TCompanion.LOGGER.error("Partial registry dump failed", t);
